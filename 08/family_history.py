...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        birth_date = people_dict[key][BIRTH_YEAR_INDEX]
        death_date = people_dict[key][DEATH_YEAR_INDEX]
        person_name = people_dict[key][NAME_INDEX]
        death_age = death_date - birth_date
        print(f'{person_name} die at {death_age} years')
...
def count_genders(people_dict):
    ...
    for key in people_dict:
        ...
        if gender == 'M':
            ...
            number_of_woman +=1
    print(f'The numbe of males is {number_of_man}')
    print(f'The numbe of females is {number_of_woman}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key in marriages_dict:
        marriage_item = marriages_dict[key]
        marriage_year = marriage_item[WEDDING_YEAR_INDEX]

        husband_key = marriage_item[HUSBAND_KEY_INDEX]
        husband_item = people_dict[husband_key]

        husband_birhdate = husband_item[BIRTH_YEAR_INDEX]
        husband_name = husband_item[NAME_INDEX]
        husband_age_wed = marriage_year - husband_birhdate

        wife_key = marriage_item[WIFE_KEY_INDEX]
        wife_item = people_dict[wife_key]
        wife_birhdate = wife_item[BIRTH_YEAR_INDEX]
        wife_name = wife_item[NAME_INDEX]
        wife_age_wed = marriage_year - wife_birhdate
        print(f'{husband_name} {husband_age_wed} > {marriage_year} < {wife_name} {wife_age_wed}')
        #print(f' for the husband {husband_name} was wed at {husband_age_wed} years old')
        #print(f' for the wife {wife_name} was wed at {wife_age_wed} years old')
